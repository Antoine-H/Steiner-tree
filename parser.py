...
def first_solution(graph,terminals):
	...
	for i in range(length(terminal)):
		for j in range(terminal.size()-i-1):
			w = nx.shortest_path_length(graph,terminal[i], terminal[j],"weight")
			too_add.append((terminal[i],terminal[j],w))
